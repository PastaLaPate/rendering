...
TextShape::TextShape(Vector2 position, const std::string &text, int fontSize,
                     Color color)
    : Shape(position), text(text), fontSize(fontSize) {
  this->color = color;
}

void TextShape::render(const Scene &scene) {
  Mat3 cameraMatrix = scene.getCamera().getCameraMatrix();
  Vector2 transformedPosition = cameraMatrix.multiply(position);
  const Camera2& cam = scene.getCamera();
  DrawTextPro(GetFontDefault(), text.c_str(), transformedPosition, {0, 0},
              cam.getRotation(), fontSize * cam.getScale().x,
              2.0f * cam.getScale().x, color);
}
...
void TextShape::setText(const std::string &newText) { text = newText; }
